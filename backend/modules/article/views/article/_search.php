...
<div class="user-search">
    ...
    <?= $form->field($model, 'title')->textInput(['class'=>'layui-input search_input', 'style' => 'width:310px;']) ?>

    <?= $form->field($model, 'status')->dropDownList(['' => '全部'] + Yii::$app->params['status']) ?>

    <?= $form->field($model, 'art_category_id')->dropDownList(
            \yii\helpers\ArrayHelper::map($category, 'art_category_id', 'category_name'),
        ['prompt'=>'全部']
    ) ?>
