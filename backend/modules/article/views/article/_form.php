...
<div class="user-update">
    <div class="user-form create_box">
        ...
        <?php $form = ActiveForm::begin(['options' => ['class' => 'layui-form form-horizontal'],'fieldConfig' => [
            'template' => "<div class='col-xs-3 col-sm-2 text-right'>{label}</div><div class='col-xs-9 col-sm-9'>{input}</div><div class='col-xs-12 col-lg-offset-2 col-xs-offset-3 col-sm-3 col-sm-offset-0'>{error}</div>",
        ]]); ?>

        <?= $form->field($model, 'art_category_id')->dropDownList(ArrayHelper::map($category,'art_category_id', 'category_name'), ['prompt'=>'请选择', 'lay-filter' => 'art_category']) ?>
        <?= $form->field($model, 'label')->dropDownList(['专题报道', '行业资讯', '市场活动']) ?>
...
<?php $this->beginBlock("goods_carousel") ?>

    $('.layui-form-select input').on('onchange', function(){
    console.log(1);
    });
...
<?php $this->endBlock() ?>
<?php $this->registerJs($this->blocks["goods_carousel"], \yii\web\View::POS_END); ?>
